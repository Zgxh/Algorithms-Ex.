...
class Solution {
    ...
    /**
     * 题目的意思是每次所有细胞状态的更新都是同时的，即每次更新要依据上一轮的存活状态来。
     * 所以要复制一份数组来存之前的存活状态，额外空间O(m*n)。在leetcode题解里看到一个O(1)的方法：
     * 增加2个状态：-1 和 2. -1 代表之前是活的，这轮死了； 2 代表之前是死的，这轮活了。这样就记录下了
     * 上一轮的状态。最后更新完之后把 -1 和 2 再还原到 0 和 1.
     *
     * @param board
     */
