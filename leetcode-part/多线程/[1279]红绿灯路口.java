...
class TrafficLight {
    ...
    private volatile boolean OneCanPass = true; // Road1 是否可以通行
    private final Object lock = new Object(); // 变灯的控制权
    ...
    public void carArrived(
        ...
        // 当要通行的 roadId 对应为绿灯时，直接通行；
        // 不同时，先变灯，然后再通行
        synchronized (lock) {
            if (roadId == 1 && OneCanPass || roadId == 2 && !OneCanPass) {
                crossCar.run();
            } else if (roadId == 1 && !OneCanPass) {
                turnGreen.run();
                OneCanPass = true;
                crossCar.run();
            } else if (roadId == 2 && OneCanPass) {
                turnGreen.run();
                OneCanPass = false;
                crossCar.run();
            }
