...
class Solution {
    ...
    /**
     * 思路：dp问题。
     * 使用dp数组储存前i天旅游所总共需要花的钱，如果该天不出行，则与前一天的花销相同；
     * 如果该天出行，则往前推1,7,30天，找最小的花销。
     * @param days
     * @param costs
     * @return
     */
    public int mincostTickets(int[] days, int[] costs) {
        ...
        for (int i = 1; i < dp.length; i++) {
            if (!daysSet.contains(i)) { // 判断该天出行与否，如果不出行，则总花销与前一天相同
                dp[i] = dp[i - 1];
                continue;
            }
            int cost1 = dp[i - 1] + costs[0];
            int cost7 = i < 7 ? costs[1] : dp[i - 7] + costs[1];
            int cost30 = i < 30 ? costs[2] : dp[i - 30] + costs[2];
